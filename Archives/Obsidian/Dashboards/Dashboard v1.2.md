...
function renderNavigation(container) {
  ...
  nav.style.justifyContent = "space-between";   // toggle left, week controls right
  nav.style.alignItems = "center";
  nav.style.marginBottom = "14px";
  nav.style.flexWrap = "wrap";

  /* ─────────────────────────────
     1️⃣  Show-Dates Toggle  (LEFT)
  ───────────────────────────── */
  ...
  switchWrapper.style.width  = "34px";
  ...
  knob.style.transform  = showDates ? "translateX(16px)" : "translateX(0)";
  ...
  /* ─────────────────────────────
     3️⃣  Assemble & Return
  ───────────────────────────── */
  nav.appendChild(toggleContainer); // left
  nav.appendChild(controls);        // right
  return nav;
}
...
function renderWeekCircles(tag, weekdaysOnly) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (skip) {
      ...
      border = `1px solid var(--divider-color, #ccc)`;
      content = showDates ? `<span style="font-size: 10px; color: ${textColor};">${date.date()}</span>` : '';
    }
...
const getCellColor = (dateStr, data) => {
  ...
  if (isToday(dateStr)) {
    return null; // defer to buildDayCell
  }
