...
function renderWeekCircles(tag, weekdaysOnly) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (isToday) {
      border = `1.8px solid ${colorPurple}`;
    }

    html += `
      <div style="display: flex; flex-direction: column; align-items: center; width: 21.6px;">
        <div style="width: 18.9px; height: 18.9px; border-radius: 50%; background: ${background}; border: ${border}; display: flex; justify-content: center; align-items: center;">
          ${content}
        </div>
...
controlRow.style.height = "28px"; // Adjust if needed
