...
function renderNavigation(container) {
  ...
  leftBtn.innerHTML = "←";
  leftBtn.setAttribute("style", buttonStyle);
  leftBtn.onclick = () => {
    weekOffset--;
  ...
  rightBtn.innerHTML = "→";
  rightBtn.setAttribute("style", buttonStyle);
  rightBtn.onclick = () => {
    weekOffset++;
    renderTiles(container);
  };
...
function renderWeekCircles(tag, weekdaysOnly) {
  ...
  for (let i = 0; i < 7; i++) {
	...
	// Always apply purple border for today (in addition)
	if (isToday) {
	  border = `2px solid ${colorPurple}`;
	}
    ...
    html += `
      <div style="display: flex; flex-direction: column; align-items: center; width: 24px;">
        <div style="width: 21px; height: 21px; border-radius: 50%; background: ${background}; border: ${border}; display: flex; justify-content: center; align-items: center;">${content}</div>
...
function renderTiles(container) {
  ...
  for (let habit of habits) {
    ...
    tile.style.boxShadow = "0 1px 5px rgba(0,0,0,0.05)";
    tile.style.background = "var(--background-primary)";

    tile.innerHTML = `
      <div style="display: flex; align-items: center; gap: 6px; margin-bottom: 4px;">
        <div style="font-size: 20px;">${habit.name}</div>
        <div style="font-size: 20px; font-weight: 600;">${streak.current}</div>
        <div style="font-size: 12px; color: var(--text-muted); margin-top: 8px;">days</div>
      </div>
      ${renderWeekCircles(habit.tag, habit.weekdaysOnly)}
      <div style="display: flex; justify-content: space-between; margin-top: 12px;">
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <div style="font-size: 11px; color: var(--text-muted);">Longest Streak</div>
          <div style="font-size: 14px;"><b>${streak.longest}</b> days</div>
        </div>
        <div style="flex: 1; display: flex; flex-direction: column; align-items: center;">
          <div style="font-size: 11px; color: var(--text-muted);">Discipline</div>
          <div style="font-size: 14px;"><b>${streak.consistency}%</b></div>
        </div>
      </div>
    `;
...
function getStreakData(tag, weekdaysOnly) {
  ...
  for (let i = 0; i < range; i++) {
    ...
    // Always track unless today and not done
