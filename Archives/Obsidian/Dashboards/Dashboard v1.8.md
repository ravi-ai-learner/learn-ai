...
let monthOffset = 0;
let showDates = true;
let showMonthly = false; // false: weekly (default), true: monthly
...
// --- View Toggle (Slider, like Show Dates) ---
function renderViewModeToggle(container) {
  ...
  toggleLabel.textContent = "Monthly View";
  ...
  switchInput.checked = showMonthly;
  ...
  slider.style.backgroundColor = showMonthly ? colorGreen : prefersDark() ? "#555" : "#ccc";
  ...
  knob.style.transform = showMonthly ? "translateX(14.4px)" : "translateX(0)";
  ...
  switchInput.addEventListener("change", () => {
    showMonthly = switchInput.checked;
    slider.style.backgroundColor = showMonthly ? colorGreen : prefersDark() ? "#555" : "#ccc";
    knob.style.transform = showMonthly ? "translateX(14.4px)" : "translateX(0)";
    // Reset view offsets when toggling view (for cleaner experience)
    weekOffset = 0;
    monthOffset = 0;
...
// --- Legend (bottom, as in original code) ---
...
// --- Navigation (centered controls, dates slider top right, monthly/weekly slider top left) ---
function renderNavigation(container) {
  ...
  // Left: Monthly/Weekly toggle (slider style)
  navWrapper.appendChild(renderViewModeToggle(container));

  // Right: Show Dates Toggle
  const toggleContainer = document.createElement("div");
  toggleContainer.style.display = "flex";
  toggleContainer.style.alignItems = "center";
  toggleContainer.style.gap = "5.4px";
  ...
  // Center: period navigation controls
  ...
  if (!showMonthly) {
    ...
    leftBtn.onclick = () => { monthOffset--; renderTiles(container); };
  }

  const centerLabel = document.createElement("span");
  centerLabel.textContent = centerText;
  centerLabel.style.fontSize = "12.6px";
  ...
  if (!showMonthly) {
    ...
    rightBtn.onclick = () => { monthOffset++; renderTiles(container); };
  }
...
function renderWeekCircles(tag, weekdaysOnly) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (isBeforeTracking) {
      ...
      border = `2px solid var(--divider-color, #ccc)`;
      content = showDates ? `<span style="font-size: 0.65em; color: ${textColor};">${date.date()}</span>` : '';
    }
    if (isToday) {
      border = `2px solid ${colorPurple}`;
    }
